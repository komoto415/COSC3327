...
public class TicTacToeGameImpl_Ng implements TicTacToeGame {
	...
	*/
    ...
    private boolean movesArrayIsWellFormed() {
        ...
        return isSubSet && noDupsExceptNeg1 && isProperLen && isRightSideFlush && noTurnsPlayedAfterWin;
    }
    ...
    private boolean noDupsExceptNeg1() {
        ...
        while (index < movesArray.length && wellFormed) {
            if (occurrences.containsKey(movesArray[index]) && movesArray[index] != NO_MOVE) {
                wellFormed = false;
            }
    ...
    private boolean noTurnsPlayedAfterWin() {
        ...
        if ((isAWinner(Player.X) || isAWinner(Player.O)) && marksPlayed < movesArray.length - ARRAY_OVERFLOW_OFFSET) {
    ...
    private boolean isRightSideFlush() {
        ...
        while (index < movesArray.length - ARRAY_OVERFLOW_OFFSET && wellFormed) {
            if (movesArray[index] == NO_MOVE && movesArray[index + 1] != NO_MOVE) {
                wellFormed = false;
            }
            index++;
        }
    ...
    public TicTacToeGameImpl_Ng(Sequence<Integer> gridPositionSequence) {
    ...
    */
	...
	/*	Example Calls:
		...
		movesArray.getMark(0, 0) = X1
		movesArray.getMark(1, 0) = O1
		movesArray.getMark(0, 1) = X2
		movesArray.getMark(1, 1) = O2
		movesArray.getMark(2, 0) = X3
		movesArray.getMark(0, 2) = O3
		movesArray.getMark(2, 1) = X4
		movesArray.getMark(2, 2) = O4
		movesArray.getMark(1, 2) = X5
		...
		movesArray.getMark(1, 0) = X1
		movesArray.getMark(0, 2) = O1
	...
	*/
    ...
    /*	Algorithm:
            ...
            Because our internal representation doesn't use row,col directly as a way of storing where marks are
            placed, we will just need to translate it into the direct index. But we also need to determine where in
            movesArray to place the element. We do this by getting the size of the set composed of the elements in
            movesArray, excluding -1. This is because this size is indiciative of the next playable insert for
            movesArray.
    */
	...
	*/
    ...
    /*	Algorithm:
            ...
            We already have 2 conditions in which we can easily check that will tell give us our return value. Both
            ...
            players is for the sake of debugging and making sure it was declaring the correct person as the having
            been the reason for the game being over or not. More on how the winnder is determined when we get to
            those methods.
    */
    ...
    public boolean isGameOver() {
        ...
        if (!gameEnded) {
            if (marksPlayed < MINIMUM_MARKS_PLAYED_TO_HAVE_A_WINNER) {
                gameEnded = false;
            } else {
                gameEnded = isAWinner(Player.X);
                if (!gameEnded) {
    ...
    */
    ...
    /*
        We are just converting our movesArray just a list a particular player's turns, while also filtering out the
    ...
    */
    ...
    private List<Integer> xTurns() {
        return IntStream.range(0, movesArray.length).filter(i -> i % 2 == 0).mapToObj(i -> movesArray[i]).filter(
                i -> i > NO_MOVE).collect(Collectors.toList());
    }
    ...
    */
    private boolean threeOfAKind(List<Integer> normalisedList) {
        ...
        boolean hasROW_COUNT = occurrences.containsValue((long) ROW_COUNT);
    ...
    */
    ...
    private int getDiagonalIndexOfRow(int row) {
        return row * (ROW_COUNT + 1);
    }

    private int[] indexToRowColumn(int index) {
        return new int[]{index / ROW_COUNT, index % ROW_COUNT};
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        List<Integer> sortedMovesList = Arrays.stream(movesArray).boxed().filter(i -> i != NO_MOVE).collect(
                Collectors.toList());
        ...
        for (int i = 0; i < ROW_COUNT * COLUMN_COUNT; i++) {
            ...
            if (i % COLUMN_COUNT == COLUMN_COUNT - 1 && i != ROW_COUNT * COLUMN_COUNT - 1) {
                sb.append('\n');
            }
        }
